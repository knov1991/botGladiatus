...
def vender_item(driver, sh):
  try:
    ...
    if len(item) > 0:
      ...
      #item_amount = item[0].get_attribute('') #TODO - validar a qtd de itens antes da venda
...
#https://s301-en.gladiatus.gameforge.com/game/ajax.php?mod=inventory&submod=move&from=-12100048&fromX=1&fromY=1&to=512&toX=1&toY=1&amount=1
#mod=inventory&submod=move&from=-12100048&fromX=1&fromY=1&to=512&toX=1&toY=1&amount=1
  ...
  try:
    ...
    if item_location:
      ...
      if len(inventory_item_check) == 0:
        url_pegar_item = CONFIG.login_data['ajax_url']+'?mod=inventory&submod=move&from='+str(item_location)+'&fromX=1&fromY=1&to=512&toX=1&toY=1&amount=1&sh='+str(sh)
        #url_pegar_item = CONFIG.login_data['ajax_url']+'?mod=inventory&submod=move&from='+str(item_location)+'&fromX=1&fromY=1&to=512&toX=1&toY=1&sh='+str(sh)
